...
namespace kakakv {
    namespace task {
        namespace ASIOSchedulerImpl {
            ...
            ElectionTimeoutImpl::ElectionTimeoutImpl(std::shared_ptr<boost::asio::deadline_timer> timerTask,Scheduler::Runnable task)
            ...
            class LogReplicationTaskImpl:public LogReplicationTask{
                friend ASIOScheduler;
            public:
                LogReplicationTaskImpl(std::shared_ptr<boost::asio::deadline_timer> timerTask,Scheduler::Runnable task,const unsigned int logReplicationInterval);
                void cancel() override;
            private:
                static void handler(const boost::system::error_code &ec,
            ...
            LogReplicationTaskImpl::LogReplicationTaskImpl(std::shared_ptr<boost::asio::deadline_timer> timerTask,Scheduler::Runnable task,const unsigned int logReplicationInterval)
            :mTimerTask(timerTask),mTask(task),logReplicationInterval(logReplicationInterval),isCancel(false),mCount(0){
            ...
            void LogReplicationTaskImpl::handler(const boost::system::error_code &ec,
                ...
                timetTask->expires_from_now( boost::posix_time::milliseconds(electionTimeoutTask->logReplicationInterval));
                // 5. 将定时任务提交给定时器线程
                timetTask->async_wait(std::bind(&ASIOSchedulerImpl::LogReplicationTaskImpl::handler, std::placeholders::_1, electionTimeoutTask));
            }
        }

        std::shared_ptr<LogReplicationTask> ASIOScheduler::scheduleLogReplicationTask(Runnable task){
            // 1. 创建定时任务
            auto timerTask = std::make_shared<boost::asio::deadline_timer>(*(this->timerIOService));
            // 2. 创建LogReplicationTaskImpl对象
            auto logReplicationTaskTimeout = std::make_shared<ASIOSchedulerImpl::LogReplicationTaskImpl>(timerTask,task,this->logReplicationInterval);
            // 3. 设置定时任务
            timerTask->expires_from_now( boost::posix_time::milliseconds(this->logReplicationDelay+this->logReplicationInterval));
            // 5. 将定时任务提交给定时器线程
            timerTask->async_wait(std::bind(&ASIOSchedulerImpl::LogReplicationTaskImpl::handler, std::placeholders::_1, logReplicationTaskTimeout));
        ...
        std::shared_ptr<ElectionTimeout> ASIOScheduler::scheduleElectionTimeout(Runnable task){
            ...
            // 5. 将定时任务提交给定时器线程
            timerTask->async_wait(std::bind(&ASIOSchedulerImpl::ElectionTimeoutImpl::handler, std::placeholders::_1, electionTimeout));
            // 6. 返回ElectionTimeoutImpl对象
