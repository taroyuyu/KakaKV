...
namespace kakakv {
    namespace net {
        ...
        void
        ASIOCodec::encapsulateMessageToByteStream(const ::google::protobuf::Message &message,std::shared_ptr<common::net::Buffer> outputBuffer) const{
            if (!outputBuffer){
                assert(outputBuffer);
            }
            //1.判断消息是否进行了初始化
