...
namespace kakakv {
    namespace net {
        ASIOChannel::ASIOChannel(std::unique_ptr<boost::asio::ip::tcp::socket> socket, std::shared_ptr<Decoder> decoder,
                ...
                mCodecBuffer(std::make_shared<common::net::CircleBuffer>(1024)), mTmpInputBufferLength(0),
        ...
        void ASIOChannel::writeRequestVote(const std::shared_ptr<const kakakv::message::RequestVote> message) {
            ...
            //3. 发送字节流
            this->sendByteStream(this->mCodecBuffer);
        }
        ...
        void ASIOChannel::sendByteStream(std::shared_ptr<common::net::CircleBuffer> buffer){
            //1. 判断是否需要更新临时输出缓冲区
            if (buffer->getUsed() > this->mTmpOutputBufferLength){
                this->mTmpOutputBufferLength = buffer->getUsed();
